...
const NAV_ITEMS = [{id:"dashboard",label:"Dashboard"},{id:"transactions",label:"Transactions"},{id:"chat",label:"AI Chat"},{id:"reminders",label:"Reminders"}];
...
function Chat({token, toast}) {
  ...
  const [loading, setLoading] = useState(false);
  const [pinning, setPinning] = useState(null); // message index being pinned
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs(m=>[...m,{
        ...
        sqlUsed:    r.sql_used   || "",   // needed for /dashboard/pin
        pinned:     false,
      }]);
    } catch(e) {
      setMsgs(m=>[...m,{role:"assistant",text:`Sorry, something went wrong: ${e.message}`}]);
    } finally { setLoading(false); }
  };

  // Calls POST /dashboard/pin — schema: { title, chart_type, sql_query, series_keys }
  const pinChart = async (idx) => {
    ...
    try {
      ...
      // Mark this message as pinned so the button changes state
  ...
  return (
    <div className="fade-up" style={{height:"100%",display:"flex",flexDirection:"column",padding:"28px",gap:"16px"}}>
      <div>
        ...
        <p style={{color:"#475569",fontSize:"13px",margin:"4px 0 0"}}>Ask anything · pin charts directly to your dashboard</p>
      </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px",display:"flex",flexDirection:"column",gap:"14px"}}>
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
              <div style={{maxWidth:"82%",display:"flex",flexDirection:"column",gap:"8px"}}>
                ...
                {m.role==="assistant" && (
                  <div style={{width:"28px",height:"28px",borderRadius:"8px",background:"linear-gradient(135deg,#10b981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"13px",marginBottom:"2px"}}>C</div>
                )}
                ...
                {m.isVisual && m.chartData?.length>0 && (
                  <div className="card" style={{padding:"16px",minWidth:"360px"}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
                      {m.chartTitle && (
                        <div style={{color:"#94a3b8",fontSize:"13px",fontWeight:"600"}}>{m.chartTitle}</div>
                      )}
                      {/* ── 📌 Pin to Dashboard button ── */}
                      {m.sqlUsed && (
                        <button
                          onClick={()=>pinChart(i)}
                          disabled={m.pinned || pinning===i}
                          className="btn"
                          style={{
                            ...
                            borderRadius:"8px",
                            cursor: m.pinned?"default":"pointer",
                          }}>
                          {pinning===i
                            ? <><span className="spin">◌</span> Pinning…</>
                            : m.pinned
                              ? "✓ Pinned"
                              : "📌 Pin to Dashboard"}
                        </button>
          ...
          {loading && (
            <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
              <div style={{width:"28px",height:"28px",borderRadius:"8px",background:"linear-gradient(135deg,#10b981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"13px"}}>C</div>
...
export default function CogniFi() {
  // Initialise directly from localStorage so a page refresh keeps the session
  ...
  return (
    <div style={{display:"flex",height:"100vh",background:"#07090f",overflow:"hidden"}}>
      ...
      <main style={{flex:1,overflowY:"auto"}}>
        ...
        {page==="chat"         && <Chat         token={token} toast={toast}/>}
