...
Table: `transactions`
  ...
  - merchant    VARCHAR      Business name
...
- User says "this year" or nothing time-related
                                -> YEAR(date) = {current_year}
- User says "this month"        -> MONTH(date) = current month AND YEAR(date) = current year.
...
If a previous attempt failed, the error is provided. Read it carefully:
  - Column not found -> check schema above and correct the column name.
...
- Rounding (e.g. "about $1,200" when the value is $1,198.50).
- Omitting minor details that are in the data but not central to the question.
- Using different but accurate phrasing ("dining" vs "Food" category).
...
def get_chat_response(db: Session, user_query: str, user_id: int) -> dict:
    ...
    if audit.get("verdict") == "FAIL":
        # Regenerate a grounded answer instead of the broken sum-all-fields hack
