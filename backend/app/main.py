...
def pay_reminder(
    ...
    """Marks a reminder as paid. Creates a transaction and rolls recursive dates forward."""
    ...
    # 1. Convert Reminder to a real Transaction entry
    new_tx = models.Transaction(
        user_id=current_user.id,
        amount=reminder.amount,
        category=reminder.category,
        merchant=reminder.merchant,
        description=f"Auto-payment for: {reminder.title}",
        type="expense",
    ...
    if not reminder.is_recursive:
        # If one-time, we just mark it as paid
        reminder.is_paid = True 
    else:
        # If recursive, add N weeks to the due_date
        weeks = reminder.frequency_weeks or 4 # Default to 4 weeks if not specified
        reminder.due_date += timedelta(weeks=weeks)
        # Note: is_paid remains False because it is now due for the next cycle
            
    db.commit()
    return {"message": "Payment recorded", "next_due": reminder.due_date}
